...
YARN_SPARK_EXECUTOR_URL = Url(join_url_dir(SPARK_YARN_URL, 'proxy', YARN_APP_ID, SPARK_REST_PATH, SPARK_APP_ID, 'executors'))
YARN_SPARK_RDD_URL = Url(join_url_dir(SPARK_YARN_URL, 'proxy', YARN_APP_ID, SPARK_REST_PATH, SPARK_APP_ID, 'storage/rdd'))
...
def test_yarn(aggregator):
    with mock.patch('requests.get', yarn_requests_get_mock) as yarn_get:
        ...
        #aggregator.assert_metric(YARN_SERVICE_CHECK,
         #   tags=['url:http://localhost:8088'])
        #assertServiceCheckOK(SPARK_SERVICE_CHECK,
        #    tags=['url:http://localhost:8088'])
...
@mock.patch('requests.get', side_effect=mesos_requests_get_mock)
def test_mesos_filter(mock_requests):
    config = {
...
def test_standalone(mock_requests):
    config = {
...
def test_standalone_pre20(mock_requests):
    config = {
...
def test_ssl_no_verify(mock_requests):
    self.run_ssl_server()
    config = {
        'instances': [SSL_NO_VERIFY_CONFIG]
    }
    self.run_check(config)

def test_ssl_cert(mock_requests):
    self.run_ssl_server()
    config = {
        'instances': [SSL_CERT_CONFIG]
    }
    self.run_check(config)
